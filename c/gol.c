...
int neighbor_count2(uint32_t* state, int x, int y) {
    uint32_t row_above = state[wrap(y-1)];
    uint32_t row_at = state[y];
    uint32_t row_below = state[wrap(y+1)];
    int rot = wrap(x-1);
    uint32_t mask_above_below = rotate_right(0xe0000000, rot);
    uint32_t mask_at = rotate_right(0xa0000000, rot);
...
int get_population2(uint32_t* state) {
    int population = 0;
    int row = 0;
